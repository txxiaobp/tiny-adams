...
Matrix::Matrix(const Point &point)
    : elem(std::vector<std::vector<double>>(point.getDimension(), std::vector<double>(1, 0)))
{
    for (int row = 0; row < point.getDimension(); row++)
    {
        elem[row][0] = point[row];
    }
}
